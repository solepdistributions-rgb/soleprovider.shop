...
/* ==================================================================
   Mobile filter drawer — body lock + misrouted-click trap.

   The summary-click toggle for filter rows is handled in theme.liquid's
   inline <script> (sets .sp-open + [open] in capture phase, calls
   stopImmediatePropagation so Dawn's form-click handler doesn't fire).
   We do NOT bind a summary click handler here — doing so created a
   duplicate that won the capture race and skipped .sp-open, breaking
   the iOS Safari [open]-recalc workaround.

   What remains here:

   1. Body scroll lock when the outer disclosure is [open]. Fixes the
      iOS Safari hit-test bug where taps on a position:fixed overlay
      get routed to (scrollY + tapY) of the underlying page.

   2. Document-level capture click trap. While the drawer is [open] on
      mobile, any click whose target lands OUTSIDE the form (e.g. on
      the outer "Filter" summary at its in-flow y-position behind the
      fixed inner panel due to lingering hit-test misrouting) gets
      swallowed. Without this, those clicks reach MenuDrawer's
      onSummaryClick on the outer summary and close the drawer.
      The X close button is exempted.
   ================================================================== */
